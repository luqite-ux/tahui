...
export function InquiryCaptchaField({
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (!requests.isCurrent(request)) return
      setError(isZh ? '验证码加载失败，请点击“换一张”重试' : 'Verification code failed to load. Select “New code” to retry.')
    } finally {
        ...
        <div className="flex h-14 w-40 items-center justify-center overflow-hidden rounded-lg border bg-slate-50">
          {challenge ? (
            ...
            <img
              ...
              alt={isZh ? '4 位验证码图片' : 'Four-character verification code'}
              width={160}
              height={56}
            />
          ) : (
            <span className="px-2 text-center text-xs text-slate-500">{loading ? (isZh ? '正在加载…' : 'Loading…') : (isZh ? '加载失败' : 'Load failed')}</span>
          )}
        </div>
